...
for a = 1:m
  ...
  [sigmat2, X] = lu(Sigmat2);
  sigmat2 = sqrt(deltat)*sigmat2;

  [sigmat3, X] = lu(Sigmat3);
  sigmat3 = sqrt(deltat)*sigmat3;

  %El ruido blanco
  N(1:15, a + 1) = pinv(eye(15) - deltat*M)*(N(1:15, a) + sigmat*DeltaW);
  DeltaW = sqrt(deltat)*(rand(15,1));

  N2(1:15, a + 1) = pinv(eye(15) - deltat*M)*(N2(1:15, a) + sigmat2*DeltaW2);
  DeltaW2 = sqrt(deltat)*(rand(15,1));

  N3(1:15, a + 1) = pinv(eye(15) - deltat*M)*(N3(1:15, a) + sigmat3*DeltaW3);
  DeltaW3 = sqrt(deltat)*(rand(15,1));
endfor
%{
%Figura 1
