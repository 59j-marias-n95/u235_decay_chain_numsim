...
for a = 1:m
  ...
  %El ruido blanco
N(:,a+1)=N(:,a)+deltat*M*N(:,a)+sigmat*(DeltaW(:,2)-DeltaW(:,1))*deltat+0.5*(sigmatY-sigmat)*(((DeltaW(:,2)-DeltaW(:,1))/sqrt(deltat))'*((DeltaW(:,2)-DeltaW(:,1))/sqrt(deltat))-1)*(ones(15,1))*deltat;
  DeltaW(:, 1) = DeltaW(1:15, 2);
  DeltaW(:, 2) = sqrt(deltat)*rand(15,1);

N2(:,a+1)=N2(:,a)+deltat*M*N2(:,a)+sigmat2*(DeltaW2(:,2)-DeltaW2(:,1))*deltat+0.5*(sigmatY2-sigmat2)*(((DeltaW2(:,2)-DeltaW2(:,1))/sqrt(deltat))'*((DeltaW2(:,2)-DeltaW2(:,1))/sqrt(deltat))-1)*(ones(15, 1))*deltat;
  DeltaW2(:, 1) = DeltaW2(1:15, 2);
  DeltaW2(:, 2) = sqrt(deltat)*rand(15,1);

N3(:,a+1)=N3(:,a)+deltat*M*N3(:,a)+sigmat3*(DeltaW3(:,2)-DeltaW3(:,1))*deltat+0.5*(sigmatY3-sigmat3)*(((DeltaW3(:,2)-DeltaW3(:,1))/sqrt(deltat))'*((DeltaW3(:,2)-DeltaW3(:,1))/sqrt(deltat))-1)*(ones(15, 1))*deltat;
